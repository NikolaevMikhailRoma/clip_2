...
# Function to split data into training, validation, and test indices
def get_data_indices(data: ImageFolder,
                     test_proportion: float = config.TEST_PROPORTION,  # Updated to use config
                     val_proportion: float = config.VAL_PROPORTION,  # Updated to use config
                     random_state: int | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Split dataset into training, validation, and test indices.
...
# Load datasets with transformations
dataset = ImageFolder(config.DATA_FOLDER, transform=test_transform)  # Updated to use config
dataset_augment = ImageFolder(config.DATA_FOLDER, transform=train_transform)  # Updated to use config

# Split dataset into train, validation, and test sets
train_indices, val_indices, test_indices = get_data_indices(dataset, random_state=config.GLOBAL_RAND_STATE)  # Updated to use config
...
# Create data loaders for each dataset split
train_dataloader = DataLoader(datasets['train'], batch_size=config.GLOBAL_BATCH_SIZE, shuffle=True)  # Updated to use config
val_dataloader = DataLoader(datasets['val'], batch_size=config.GLOBAL_BATCH_SIZE, shuffle=True)  # Updated to use config
test_dataloader = DataLoader(datasets['test'], batch_size=config.GLOBAL_BATCH_SIZE, shuffle=True)  # Updated to use config
